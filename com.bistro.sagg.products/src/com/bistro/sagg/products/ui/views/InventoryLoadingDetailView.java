...
import com.bistro.sagg.products.ui.viewers.OrderItemTableLabelProvider;
...
public class InventoryLoadingDetailView extends ViewPart {

	/**
	 * The ID of the view as specified by the extension.
	 */
	public static final String ID = "com.bistro.sagg.products.ui.views.InventoryLoadingDetailView";
...
//			.lookup(ProductServices.class.getName());
	...
	public InventoryLoadingDetailView() {
		super();
		this.bundleContext = FrameworkUtil.getBundle(InventoryLoadingDetailView.class).getBundleContext();
...
//		productsTableViewer.setSorter(new MarketableProductListSorter());
		...
		productsTable.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(source.getSelectionCount() > 0) {
					...
					selectedItem = (PurchaseOrderItem) ((StructuredSelection)productsTableViewer.getSelection()).getFirstElement();
		...
		TableColumn unitPriceColumn = new TableColumn(productsTable, SWT.NONE);
		unitPriceColumn.setWidth(100);
		unitPriceColumn.setText("Precio Unitario");
		
		TableColumn totalColumn = new TableColumn(productsTable, SWT.NONE);
...
//					selectedItem.recalculateAmount();
		...
		confirmItemListButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				Employee receiver = SaggSession.getCurrentSession().getSessionObject(SaggSessionConstants.ACTIVE_USER);
				List<PurchaseOrderItem> items = new ArrayList<PurchaseOrderItem>();
				for (TableItem item : productsTable.getItems()) {
					items.add((PurchaseOrderItem) item.getData());
				}
				PurchaseOrder order = orderServices.createPurchaseOrder(branch, selectedSupplier, receiver, items);
				
				clearItemListButton.setEnabled(false);
				confirmItemListButton.setEnabled(false);
				
				Map<String,Object> properties = new HashMap<String, Object>();
		        properties.put(ProductsCommunicationConstants.PURCHASE_ORDER_DATA, order);
		        properties.put(ProductsCommunicationConstants.ADD_PRODUCT_SUPPLIER_DATA, selectedSupplier);
				Event event = new Event(ProductsCommunicationConstants.CONFIRM_PURCHASE_ORDER_EVENT, properties);
...
//		confirmItemListButton.setEnabled(false);
	...
	private void createAddProductEventHandler(Composite parent, TableViewer productsTableViewer) {
		EventHandler handler = new EventHandler() {
			public void handleEvent(final Event event) {
				PurchaseOrderItem item = OrderItemFactory.createPurchaseOrderItem(
						(Product) event.getProperty(ProductsCommunicationConstants.ADD_PRODUCT_DATA),
						(int) event.getProperty(ProductsCommunicationConstants.ADD_PRODUCT_QUANTITY_DATA),
						(BigDecimal) event.getProperty(ProductsCommunicationConstants.ADD_PRODUCT_UNIT_PRICE_DATA));
				selectedSupplier = (Supplier) event.getProperty(ProductsCommunicationConstants.ADD_PRODUCT_SUPPLIER_DATA);
				if (parent.getDisplay().getThread() == Thread.currentThread()) {
	    ...
	    Dictionary<String,String> properties = new Hashtable<String, String>();
	    properties.put(EventConstants.EVENT_TOPIC, ProductsCommunicationConstants.ADD_PRODUCT_EVENT);
	    bundleContext.registerService(EventHandler.class, handler, properties);
	}
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				InventoryLoadingDetailView.this.fillContextMenu(manager);
			}
		});
	}
